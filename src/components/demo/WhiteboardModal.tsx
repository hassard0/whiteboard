...
import { X, Pencil, Highlighter, Trash2, Palette, ChevronDown, Minus, Plus, Wand2, Download, Loader2, Sparkles, ImagePlus } from "lucide-react";
...
export function WhiteboardModal({ diagrams, onClose }: WhiteboardModalProps) {
  ...
  useEffect(() => {
    if (!bgImageUrl) { bgImageRef.current = null; return; }
    const img = new Image();
    img.onload = () => { bgImageRef.current = img; drawDiagram(); };
    img.src = bgImageUrl;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bgImageUrl]);
  ...
  const drawDiagram = useCallback(() => {
    ...
    img.onload = () => {
      ...
      if (bgImageRef.current) {
        ...
        ctx.globalAlpha = 0.35;
        ctx.drawImage(bi, (w - bi.width * s) / 2, (h - bi.height * s) / 2, bi.width * s, bi.height * s);
        ctx.globalAlpha = 1;
      }
  ...
  // ─── Redraw when diagram selection, svgStore, bg, or bgImage changes ──────
  // Guard against redrawing mid-stroke (would wipe the background visually)
  useEffect(() => {
    if (!isDrawingRef.current) {
      drawDiagram();
    }
  }, [svgStore, selectedDiagramIdx, bg, bgImageUrl, drawDiagram]);
  ...
  // Only show "rendering" if there IS a diagram selected that hasn't rendered yet
  const currentSvgReady = selectedDiagram?.diagram
    ? !!(svgStore[selectedDiagram.id])
    : true; // blank canvas or no diagram = always "ready"
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border/40 bg-card/80 flex-wrap">
            ...
            <div className="relative">
              ...
              />
              <button
                title="Upload image as background"
                onClick={() => bgUploadRef.current?.click()}
                className={cn(
                  "flex items-center gap-1.5 rounded-lg border bg-background px-2.5 py-1.5 text-xs transition-colors",
                  bgImageUrl
              ...
              >
                ...
                <span className="hidden sm:inline">{bgImageUrl ? "Change BG" : "BG Image"}</span>
              </button>
              {bgImageUrl && (
                <button
                  title="Remove background image"
                  onClick={() => { setBgImageUrl(null); bgImageRef.current = null; drawDiagram(); }}
                  className="absolute -top-1.5 -right-1.5 h-4 w-4 rounded-full bg-destructive text-destructive-foreground flex items-center justify-center hover:bg-destructive/80 transition-colors"
                >
